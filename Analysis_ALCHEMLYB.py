...
class NCFilePostProcessing:
    ...
    def __init__(
        ...
        # Let us define the phase
        self.phase = nc_file.stem

        logger.info(f"Let us start alchemical analysis for {self.phase.upper()} phase")
        
        # Let us initialize the dataset
        self.dataset = nc.Dataset(nc_file)

        # Let us define T and skiplambda
        self.__T = T
        self.__skiplambda = skiplambda

    def extract_standard_correction(self) -> tuple[float, float]:
        
        logger.info("Extracting standard correction...")
        
        # Extract standard correction
        raw_standard_correction = self.dataset.groups['metadata'].variables.get('standard_state_correction', None)
        ...
        if raw_standard_correction is not None:
            ...
            standard_correction_kT : float = -np.frombuffer(binary_data, dtype=np.float64)[0] # Positive
    ...
    def extract_lambdas(thermodynamic_states : nc.Group) -> pd.DataFrame:
        ...
        logger.info("Extracting thermodynamic states...")

        # Define the order of lambda values
        ...
        logger.info(f"{len(rows)} lambdas extracted")
    ...
    def extract_u_nk(
        ...
        logger.debug(f"Extracting energies for {current_lambda} state. Shape: {array.shape}")
        
        """Tu función extract_u_nk original, sin cambios"""
    ...
    def extract_yank_to_unk(self) -> list:
        ...
        """
        Extrae datos de un archivo NetCDF de YANK y los convierte a formato u_nk de alchemlyb.
        
        Parameters
        ----------
        nc_file : Path
            Ruta al archivo .nc de YANK
        phase : str
            Nombre de la fase ('complex', 'solvent', etc.)
        
        Returns
        -------
        u_nk_list : list of pd.DataFrame
            Lista de DataFrames u_nk, uno por estado lambda
        """
        ...
        # Extraer los estados (para saber qué lambda corresponde a cada estado)
        # YANK guarda esto en 'states' que tiene las fracciones lambda
        lambdas = self.extract_lambdas(self.dataset.groups['thermodynamic_states'])
        
        # Crear lista de u_nk, uno por cada estado lambda
        u_nk_list = []
        ...
        if len(labels) == 1:
            columns = [lambda_dict[labels[0]][i] for i in range(len(lambdas)) if (i != self.__skiplambda or self.__skiplambda is None)]
        else:
            columns = [
                tuple([lambda_dict[key][i] for key in labels])
                for i in range(len(lambdas)) if (i != self.__skiplambda or self.__skiplambda is None)
            ]

        for state_idx in range(n_states):

            if state_idx != self.__skiplambda:
                # Para cada iteración, encontrar qué réplica estaba en state_idx
                # states shape: (n_iterations, n_replica)
                ...
                # Energías de esa réplica en cada iteración, evaluadas en todos los estados
                # shape: (n_iterations, n_states)
                u_k = energies[valid_iterations, replicas_clean, :]

                # Restar la energía del propio estado (energías relativas)
                u_k -= u_k[:, state_idx].reshape(-1, 1)

                # ✅ Eliminar la columna del estado saltado DESPUÉS de restar
                if self.__skiplambda is not None:
                    u_k = np.delete(u_k, self.__skiplambda, axis=1)  # ahora shape: (n_iterations, n_states-1)
                
                # Crear Series de tiempo
                time = pd.Series(data=valid_iterations, name='time', dtype="Float64")
                
                # Lambda actual para este estado
...
class ABFE_YANK(WorkflowBase,NCFilePostProcessing):
    ...
    def __init__(
        ...
        WorkflowBase.__init__(self,unit=units, software=software, T=T, out=outdirectory)
        NCFilePostProcessing.__init__(self,Path(dir).joinpath(f"{prefix}.{suffix}"),T, skiplambda)
    ...
    def read(
        ...
        if read_u_nk:
            ...
            if self.software == 'OPENMM':
                ...
                u_nk_list = joblib.Parallel(n_jobs=n_jobs)(
                    joblib.delayed(extract_u_nk)(self._extract_u_nk, Path(file), self.T)
                    for file in self.file_list
                )
